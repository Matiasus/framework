  ...
  */
  private function handler()
  {
    ...
    if (empty($this->instance)) {
      // throw to exception with error message
      throw new \Exception("[".get_called_class()."]:[".__LINE__."]: Can't hand to objects to Controller!"); 
    }
    // if method exists
    if (!method_exists($this->instance, self::METHOD)) {
      // throw to exception with error message
      throw new \Exception("[".get_called_class()."]:[".__LINE__."]: Method '".self::METHOD."' doesn't exists!"); 
    }
    // loop through objects get from contructor
    foreach ($this->container as $object) {
      // store all objects to controller
      $this->instance->set(get_class($object), $object);
    }
  }
